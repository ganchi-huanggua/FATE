...
class DualPrompt(VisionTransformer):
    ...
    def extract(self, x: torch.Tensor, simclr=False, ce=True):
    ...
    def forward(self, x, only_fc=False, only_feat=False, **kwargs):
        if only_fc:
            return self.head(x)
        x = self.extract(x, simclr=kwargs['simclr'], ce=kwargs['ce'])
        if self.global_pool:
...
def vpt_on_vit_base_patch_16_224(pretrained=True, pretrained_path='/home/lhz/code/semi-pt/vit_base_patch16_224'
    ...
    model_kwargs = dict(prompt_length=12, prompt_init='uniform', insert_layers=[0], VPT_type="shallow")
